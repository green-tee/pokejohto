...
static u8 CreateNPCTrainerParty(struct Pokemon *party, u16 trainerNum)
{
    ...
    u8 fixedIV;
    s32 i, j;
    enum Shininess shininess = SHINY_RANDOM;
     ...
     && !(gBattleTypeFlags & (BATTLE_TYPE_BATTLE_TOWER | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_TRAINER_TOWER)))
    {
        ...
        for (i = 0; i < gTrainers[trainerNum].partySize; i++)
        {
            ...
            case 0:
            {
                ...
                shininess = TRAINER_MON_SHININESS(partyData[i]);
                personalityValue += nameHash << 8;
                fixedIV = partyData[i].iv * MAX_PER_STAT_IVS / 255;
                CreateMon(&party[i], partyData[i].species, partyData[i].lvl, fixedIV, TRUE, personalityValue, OT_ID_RANDOM, 0, shininess);
                break;
            }
...
static void HandleEndTurn_FinishBattle(void)
{
    ...
    {
        if (!(gBattleTypeFlags & (BATTLE_TYPE_TRAINER_TOWER | BATTLE_TYPE_EREADER_TRAINER | BATTLE_TYPE_CATCHING_TUTORIAL | BATTLE_TYPE_BATTLE_TOWER | BATTLE_TYPE_SAFARI | BATTLE_TYPE_FIRST_BATTLE | BATTLE_TYPE_LINK)))
        {
            for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++)
            {
                if (GetBattlerSide(gActiveBattler) == B_SIDE_PLAYER)
